...
t.describe("util paths", function()
  ...
  t.it("resolves symlinks so one directory has one name", function()
    ...
    if util.is_windows then
      -- Windows needs the directory flag; without it libuv creates the wrong
      -- symlink type and realpath behaviour is not representative.
      linked = vim.uv.fs_symlink(real .. "/site", link, { dir = true })
    else
      linked = vim.uv.fs_symlink(real .. "/site", link)
    end
